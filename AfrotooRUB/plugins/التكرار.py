...
async def spam_function(event, JoKeRUB, l313l, sleeptimem, sleeptimet, DelaySpam=False):
    ...
    if len(l313l) == 2:
        ...
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            AfrotooUB = await event.client.send_file(
                event.chat_id, AfrotooUB, caption=AfrotooUB
            )
        ...
        if BOTLOG:
            ...
            JoKeRUB = await event.client.send_file(BOTLOG_CHATID, AfrotooUB)
            await _catutils.unsavegif(event, AfrotooUB)
        return
