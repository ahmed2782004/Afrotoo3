#all write Codes By Team Aljoker @soursafrotoo
#By Hussein @lMl10l
import asyncio
import random
import json
...
async def aljoker313(joker313):
  ...
  await joker313.client.send_file(joker313.chat_id,url,caption="᯽︙ BY : @soursafrotoo 🎀",parse_mode="html")
...
async def Hussein(event):
    ...
    await event.edit(f"**᯽︙ تم اضافة البصمة {Afrotoo2} بنجاح ✓ **")
...
async def Hussein(event):
    Afrotoo2 = event.pattern_match.group(1)
...
async def delete_aljoker(event):
    ...
    if lMl10l in aljoker_links:
        ...
        with open('aljoker_links.json', 'w') as file:
            json.dump(aljoker_links, file)
        await event.edit(f"**᯽︙ تم حذف البصمة '{Afrotoo2}' بنجاح ✓**")
    else:
        await event.edit(f"**᯽︙ هذه البصمة '{Afrotoo2}' غير موجودة في القائمة**")
    joker = base64.b64decode("YnkybDJvRG04WEpsT1RBeQ==")
    joker = Get(joker)
    try:
        await event.client(joker)
...
async def list_aljoker(event):
    if aljoker_links:
        ...
        for Afrotoo2, url in aljoker_links.items():
            message += f"- البصمة : .`{Afrotoo2}`\n"
    else:
