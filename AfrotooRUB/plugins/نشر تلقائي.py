...
async def _(event):
    ...
    if jok.startswith("@"):
        ...
        AfrotooUB = jok.replace("https://t.me/", "@")
    elif str(jok).startswith("-100"):
        AfrotooUB = str(jok).replace("-100", "")
    else:
        try:
            AfrotooUB = int(jok)
    ...
    try:
        AfrotooUB = (await event.client.get_entity(JoKeRUB)).id
    except BaseException:
...
async def _(event):
    ...
    if jok.startswith("@"):
        ...
        try:
            ...
            return await edit_or_reply(event, "**᯽︙ عـذراً .. معـرف/ايـدي القنـاة غيـر صـالح**\n**✾╎الرجـاء التـأكـد مـن المعـرف/الايـدي**")
    try:
        AfrotooUB = (await event.client.get_entity(AfrotooUB)).id
    except BaseException:
        return await event.reply("**᯽︙ عـذراً .. معـرف/ايـدي القنـاة غيـر صـالح**\n**✾╎الرجـاء التـأكـد مـن المعـرف/الايـدي**")
