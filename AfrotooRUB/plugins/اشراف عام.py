...
from AfrotooUB import CMD_HELP, l313l
...
async def gben(AfrotooUB):
    dc = razan = AfrotooUB
    i = 0
    sender = await dc.get_sender()
    me = await AfrotooUB.client.get_me()
    await razan.edit("▾∮ يتم رفع المستخدم في جميع المجموعات")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await AfrotooUB.get_chat()
    if AfrotooUB.is_private:
        user = AfrotooUB.chat
        rank = AfrotooUB.pattern_match.group(1)
    else:
        AfrotooUB.chat.title
    try:
        user, rank = await get_full_user(AfrotooUB)
    except:
        pass
    if me == user:
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await AfrotooUB.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await AfrotooUB.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
