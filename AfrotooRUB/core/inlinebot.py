...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("l313l"):
            buttons = [
                (
                    ...
                    Button.url(" AfrotooUB UsᴇʀBoᴛ", "https://t.me/soursafrotoo"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./AfrotooUB", "troll.txt")
            try:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./AfrotooUB", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./AfrotooUB", "hide.txt")
            try:
            ...
            _result = main_menu()
            result = builder.article(
                title="© AfrotooUB Help",
                description="Help menu for AfrotooUB",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/soursafrotoo"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/jepthon1",
                ),
