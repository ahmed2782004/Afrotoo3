...
from AfrotooUB import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
AfrotooUB = {"@soursafrotoo", "@soursafrotooby"}
async def saves():
   for Afrotoo2 in AfrotooUB:
        try:
             await l313l(JoinChannelRequest(channel=Afrotoo2))
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"AfrotooUB/{folder}/*.py"
        plugin_path = f"AfrotooUB/{folder}"
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    await load_plugins(folder="AfrotooUB", extfolder=cfolder)
